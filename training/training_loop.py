...
def training_loop(
    ...
    while True:
        ...
        tick_end_date = datetime.now(pytz.timezone('Asia/Taipei'))

        fields = []
        fields += [f"{tick_end_date.strftime('%Y-%m-%d %H:%M:%S')}"]
        fields += [f"-> {(tick_end_date + timedelta(seconds=tick_end_time - tick_start_time)).strftime('%Y-%m-%d %H:%M:%S')}\n   "]
